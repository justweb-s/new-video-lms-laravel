...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background:#f7fafc; padding:24px; color:#111827;">
    <div style="max-width:640px; margin:0 auto; background:#ffffff; border-radius:8px; padding:24px; box-shadow:0 1px 2px rgba(0,0,0,0.06);">
        ...
        <p style="margin:0 0 12px 0; line-height:1.6;">hai ricevuto una gift card per il corso <strong>{{ $giftCard->course->name }}</strong> su {{ config('app.name') }}.</p>
        ...
        <div style="margin:16px 0; padding:12px 16px; background:#f9fafb; border-radius:6px; border:1px solid #e5e7eb;">
            ...
            <p style="margin:0; font-size:18px; font-weight:700; letter-spacing:1px;">{{ $giftCard->code }}</p>
        </div>
        ...
        <div style="margin:24px 0; text-align:center;">
            <a href="{{ route('giftcards.redeem', ['code' => $giftCard->code]) }}" style="display:inline-block; background:#0ea5e9; color:#fff; text-decoration:none; padding:12px 20px; border-radius:6px; font-weight:600;">Riscatta ora</a>
        </div>
